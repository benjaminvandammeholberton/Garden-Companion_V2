...
const Recommandations = () => {
  ...
  useEffect(() => {
    const fetchVegetables = async () => {
      try {
        setIsLoading(true);
        const response = await axiosInstance.get("/api/v1/vegetable_info/");
  ...
  return (
    <div className="grid grid-cols-4 px-2 h-[280px] mt-[-5px] overflow-auto scrollbar-thin scrollbar-thumb-rounded-full scrollbar-track-rounded-full scrollbar-thumb-slate-400 scrollbar-track-gray-100 dark:scrollbar-thumb-gray-100 dark:scrollbar-track-slate-900">
      {listVegetables?.map((veg) => (
        <div key={veg.vegetable_info_id} className="flex flex-col items-center">
          <Dialog modal>
            ...
            <DialogContent className="sm:max-w-[625px]">
              ...
              <div className="grid grid-cols-2 mx-auto md:grid-cols-4 gap-2 md:gap-4 py-2 md:py-4">
                <Card className="gap-2 p-2 text-center gap-2 text-sm flex flex-col justify-between items-center md:w-32">
                ...
                <Card className="gap-2 p-2 text-center text-sm flex flex-col justify-between items-center md:w-32">
                  ...
                  <div className="flex flex-col leading-none">
                    ...
                    <span className="text-md md:text-lg font-bold  leading-none">
                      {getFrostResistanceDisplay(veg.cold_resistance)}
                    </span>
                  </div>
                </Card>
              </div>
